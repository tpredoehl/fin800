...
\begin{document}
	...
	\begin{itemize}
		\item N=2 assets, a riskless asset and a risky \replaced[id=tp]{asset}{one}
		\item Frictionless markets \replacedTP{1}{2}
		...
		\item $W_{t} = $ investable wealth (endowment at time t \replaced[id=tp]{after consumption $c_t$ took place}{minus consumption $c_t$})
		\item  $U_{t}(W_{t+1}) = -e^{-aW_{t+1}-bESG_t}$
		\item $a > 0 $ (the \replaced[id=tp]{agent}{investor} has positive absolute risk aversion)
