...
\begin{document}
	...
	\todo{here we need to elaborate!}
	\highlight[id=tp]{I think here we need to expand on how the agent actually uses the ESG information. I believe that asset universe selection is key in this consideration:}		
	\begin{itemize}
			\item agent defines Minimum Acceptable ESG Rating: MAER
			\item if $\mu_{x, ESG} < MAER$ exclude from universe
			\item if $\sigma_{ESG}^2 > \sigma_{MAER}^2$ exclude from universe
			\item 
	\end{itemize}
	
	\replaced{adasd}{sfsdf}
