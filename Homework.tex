...
\begin{document}
	...
	\addedTP{Note that $\lambda = f(c_t = f(y_t))$. I would recommend to align the argument along IMRS and IES. Also note that a measure of risk-aversion $\gamma$ is quite pointless when faced only with risk-less alternatives. I see $\gamma$ as merely affecting the IES and we can state the partial derivative of $\frac{\partial \ln \left(\frac{c_{t+1}}{c_t}\right)}{\partial IES} = \ln \delta + \ln (1+R)$}
