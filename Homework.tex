...
\begin{document}
	...
	\replacedTP{A few observations on}{Observations on optimal portfolio weights} \ref{Optimal weight}:
	\begin{itemize}
		\item \addedTP{$\frac{\partial \theta_t}{\partial a} < 0$ allocation to risky assets decreases in $a$} 
		\item \addedTP{$\frac{\partial \theta_t}{\partial W_t} < 0$ allocation to risky assets decreases in $W_t$}
