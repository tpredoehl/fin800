...
\begin{document}
	...
	\replacedTP{	By noting that: i) $W_t$ and ii) $r_f$ are known in t and the expectation (conditional to t) of the expected return of the risky asset corresponds to the expected return itself by the law of iterated expectations, we can find that:   }{Given $E_t[W_t]=W_t$, $r_f=\text{const.}$ and $E_t[E_t[r_{t+1}]]$ by LIE:} 
