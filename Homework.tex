...
\begin{align*}
	...
	&= (w_t - c_t)[R_f + \theta_t (R_{t+1} - R_f) ]\\
		...
		[R_f + \theta_t (R_{t+1} - R_f) ] &= \text{return on investment in period t+1}
\end{align*}
...
\begin{align*}
	...
	&= \max_{(c_t, \theta_t)} \left[u(c_t) + \delta E_t\left[J_{t+1}((w_t - c_t)[R_f + \theta_t (R_{t+1} - R_f) ]) \right])\right]\right]
\end{align*}
