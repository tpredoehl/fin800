...
\begin{document}
	...
	\begin{align*}
		w_0 &= y_t + \frac{y_{t+1}}{1+R}\\
		c_t &= w_0 - \frac{y_{t+1}}{1+R} - \lambda\\
		c_{t+1} &= y_{t+1} + \lambda(1+R)\\
		y_{t+1} &= c_{t+1} - \lambda(1+R)\\
		c_t &= w_0 - \frac{c_{t+1} - \lambda(1+R)}{1+R} - \lambda\\
		c_t &= w_0 - \frac{c_{t+1}}{1+R}\\
		c_t (1+R) &= w_0 - c_{t+1}\\
		(1+R) &= \frac{w_0}{c_t} - \frac{c_{t+1}}{c_t}\\
		\frac{c_{t+1}}{c_t} &= \frac{w_0}{c_t} - (1+R)
	\end{align*}
	
	
	The optimal portfolio is determined by the agents choice of $c_t$ Inserting the budget constraints into the optimality condition
	\begin{align*}
		...
		\lambda  &= \frac{\delta^{\frac{1}{\gamma}}(1+R)^{\frac{1}{\gamma}} y_t - y_{t+1}}{(1+R) + \delta^{\frac{1}{\gamma}}(1+R)^{\frac{1}{\gamma}}}\\
	\end{align*}
	
	The optimal portfolio $\lambda$ is sensitive to changes in the ratio $\frac{c_{t+1}}{c_t} = \delta^{\frac{1}{\gamma}} (1+R)^{\frac{1}{\gamma}}$ and this ratio is sensitive to changes in $\gamma$\footnote{I fail to produce the direct $\frac{\partial \lambda}{\partial \gamma}$ \alert{Any ideas?}}. 
	\begin{align*}
	...
	A decrease \footnote{Interesting question: What is the direction of causality? Do agents reduce risk aversion, when they observe higher returns or do returns increase when agents happen to have lower risk aversion?} in risk aversion $\gamma$, i.e. an increase of the IES $\frac{1}{\gamma}$ increases the preference for future consumption by a percentage equal to the sum personal time preference $\delta$ and asset return R. The less risk averse agent accepts to reduce current consumption and invests more in the risky asset. 
