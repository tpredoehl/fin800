...
\begin{align*}
	...
	\frac{\partial \ln \left(\frac{c_{t+1}}{c_t}\right)}{\partial \ln IMRS} &= \rho\\
	IMRS: \frac{u_{c_{t+1}}'}{u_{c_t}'} &= \delta \left(\frac{c_{t+1}}{c_t}\right)^{-1/\rho}\\
	\left(\frac{u_{c_{t+1}}'}{u_{c_t}'}\right)^{-\rho} &= \delta^{-\rho} \left(\frac{c_t{+1}}{c_t}\right)\\
	\delta^\rho \left(\frac{u_{c_{t+1}}'}{u_{c_t}'}\right)^{-\rho} &= \left(\frac{c_t{+1}}{c_t}\right)\\
	\rho \ln \delta - \rho \ln \left(\frac{u_{c_{t+1}}'}{u_{c_t}'}\right) &= \ln \left(\frac{c_t{+1}}{c_t}\right)\\
\end{align*}
