...
\begin{document}
	...
	\listofchanges[]
	% I also replace all r_t,{t+1} with r_{t+1} and r_f with r_f to improve legibility
	...
	\begin{itemize}
		...
		\item The risky asset returns $r_{\deletedTP{t,}t+1} = \mu + \beta x_t + \sigma_r \eta_{r, t+1}$ \addedTP{, with $r_{t+1} \sim N(\mu + \beta x_t, \sigma_r^2)$}
		...
		\item $a > 0 $ (the \replacedTP{investor}{agent} has positive absolute risk aversion)
		\item The \replacedTP{investor}{agent} knows the distribution of the return, the underlying parameters and the ESG score of the risky asset.
	\end{itemize}
	...
	\begin{align*}
		W_{t+1} &= (W_t)\left[(1-\theta_t)(1+r_f)+ \theta_t(1+r_{t,t+1}) \right]\\
		&= (W_t) \left[1 -\theta_t + r_f - \theta_t r_f + \theta_t + \theta_t r_{t,t+1}\right]\\
		&= (W_t) \left[1 + r_f + \theta_t(r_{t,t+1}-r_f) \right]
	...
	\begin{align*}
		\max_{(\theta_t)} E_t[U_{t}(W_{t+1})] &= E_t[-e^{(-aW_t) [1 + r_f + \theta_t(r_{t,t+1}-r_f)] - bESG_t}] \\
	...
	\begin{align*}
		\max_{(\theta_t)} E_t[U_{t}(W_{t+1})] &= -e^{(-aW_t) [1 + r_f + \theta_t(E_{t}[r_{t,t+1}]-r_f)] - bESG_t + \frac{1}{2}a^2W_t^2\theta_t^2\sigma_r^2} \\
	...
	\begin{align*}
		(-aW_t) [1 + r_f + \theta_t(E_{t}[r_{t,t+1}]-r_f)] - bESG_t + \frac{1}{2}(a^2W_t^2\theta_t^2\sigma_r^2) \\
	\end{align*} \vspace{-3em}
	
	The first-order condition (FOC) is: \vspace{-2.5em}\\
	
	\begin{align*}
		0 &= (-aW_t)(E_{t}[r_{t,t+1}]-r_f)) + (a^2W_t^2\theta_t\sigma_r^2) \\
	\end{align*} \vspace{-3em}
	
	The optimal portfolio that solves the FOC is: 
	
	\begin{equation} \label{Optimal weight}
		\theta_t^* = \frac{1}{aW_t}\frac{(E_{t}[r_{t,t+1}]-r_f)}{\sigma_r^2} = \frac{1}{aW_t\sigma_r}Sharpe\\
	\end{equation}
	...
	\begin{itemize}
		...
		\item Assuming $E_{t}[r_{t,t+1}] - r_f$ is positive the Reader can infere that, regardless of the actual preferences of the subject, any investor will keep some holdings of the risky asset in the optimal portfolio.\\ \vspace{-1.5em}    
	\end{itemize}
	...
	\begin{align*}
		W_{t+1}^{*} &= (W_t) \left[1 + r_f + \theta_t^*(r_{t,t+1}-r_f) \right]\\
		&= (W_t) \left[1 + r_f + \frac{1}{(aW_t)}\frac{(E_{t}[r_{t,t+1}]-r_f)}{\sigma_r^2}(r_{t,t+1}-r_f) \right]\\
		&= (W_t)(1 + r_f) + \frac{1}{a}\frac{(E_{t}[r_{t,t+1}]-r_f)}{\sigma_r^2}(r_{t,t+1}-r_f)
	\end{align*} \vspace{-1.35em}
	
	The mean of $W_{t+1}^{*}$ becomes:
	\begin{align*}
		E_{t}[W_{t+1}^{*}] = E_{t}[(W_t)(1 + r_f)] + E_{t}[\frac{1}{a}(\frac{E_{t}[r_{t,t+1}]-r_{f, t+1)}}{\sigma_r^2}(r_{t,t+1}-r_f)]
	...
	\begin{align} \label{Mean Wealth}
		E_{t}[W_{t+1}^{*}] &= W_t(1 + r_f) + \frac{1}{a}\frac{(E_{t}[r_{t,t+1}]-r_f)^2}{\sigma_r^2} \notag\\
	...
	\begin{align*}
		V_{t}(W_{t+1}^{*}) = V_{t}[W_t(1 + r_f) + W_t[\frac{1}{a}\frac{(E_{t}[r_{t,t+1}]-r_f)}{\sigma_r^{2}}(r_{t,t+1}-r_f)]]
	\end{align*} \vspace{-1.75em}
	...
	\begin{align} %\label{Variance Wealth}
		V_{t}(W_{t+1}^{*}) &= (W_t^2)[\frac{1}{(W_t^2a^2)}\frac{(E_{t}[r_{t,t+1}]-r_f)^2}{\sigma_r^4}\sigma_r^2] \notag \\
	...
	\begin{align*}
		E_t[U_{t}(W_{t+1}^{*})] &= -e^{(-aW_t) [1 + r_f + \theta_t^*(E_{t}[r_{t,t+1}]-r_f)] - bESG_t + \frac{1}{2}a^2W_t^2(\theta_t^{*})^2\sigma_r^2} \\
		&= -e^{-a [W_t(1 + r_f) + W_t(\theta_t^*(E_{t}[r_{t,t+1}]-r_f)) + \frac{b}{a}ESG_t - \frac{1}{2}aW_t^2(\theta_t^{*})^2\sigma_r^2]}
	...
	\begin{itemize}
		\item There are now two stochastic terms in the utility function of the investor: $ESG_t$ and $r_{t,t+1}$ (the latter embedded in $W_{t+1}$).\\ Therefore, the mean and the variance of the argument: $(-aW_t) [1 + r_f + \theta_t(r_{t,t+1}-r_f)] - bESG_t$ are
		\subitem Mean: $(-aW_t) [1 + r_f + \theta_t(E[r_{t,t+1}]-r_f)] - b\mu_{ESG}$
	...
	\begin{align*}
		\max_{(\theta_t)} E_t[U_{t}(W_{t+1})] &= E_t[-e^{(-aW_t) [1 + r_f + \theta_t(r_{t,t+1}-r_f)] - bESG_t}] \\
		&= -e^{(-aW_t) [1 + r_f + \theta_t(r_{t,t+1}-r_f)] - b\mu_{ESG} +\frac{1}{2}a^2W_t^2\theta_t^2\sigma_r^2 + \frac{1}{2}b^2\sigma_{ESG}^2}]
	\end{align*} \vspace{-2em}
	
	As before, the solution is obtained by maximizing the argument of the above function: \vspace{-0.5em}
	\begin{align*}
		(-aW_t) [1 + r_f + \theta_t(r_{t,t+1}-r_f)] - b\mu_{ESG} +\frac{1}{2}a^2W_t^2\theta_t^2\sigma_r^2 + \frac{1}{2}b^2\sigma_{ESG}^2
	\end{align*}
	The FOC becomes:
	\begin{align*}
		0 = (-aW_t)(E_{t}[r_{t,t+1}]-r_f)) + (a^2W_t^2\theta_t\sigma_r^2)
	...
	\begin{align*}
		E_t[U_{t}(W_{t+1}^{*})] &= -e^{(-aW_t) [1 + r_f + \theta_t^*(E_{t}[r_{t,t+1}]-r_f)] - b\mu_{ESG} + \frac{1}{2}a^2W_t^2(\theta_t^*)^2\sigma_r^2 +\frac{1}{2}b^2\sigma_{ESG}^2 } \\
		&= -e^{-a [W_t(1 + r_f) + W_t(\theta_t^*(E_{t}[r_{t,t+1}]-r_f)) + \frac{b}{a}\mu_{ESG} - \frac{1}{2}aW_t^2(\theta_t^{*})^2\sigma_r^2 -b\frac{b}{2a}\sigma_{ESG}^2]}
	\end{align*} %\vspace{-1em}
	The argument represents the CE of the investor: 
	\begin{align*}
		CE^* &= [W_t(1 + r_f) + W_t(\theta_t^*(E_{t}[r_{t,t+1}]-r_f)) + \frac{b}{a}\mu_{ESG} - \frac{1}{2}aW_t^2(\theta_t^{*})^2\sigma_r^2 -\frac{b^2}{2a}\sigma_{ESG}^2] \\
