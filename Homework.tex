...
\begin{document}
	...
	\begin{itemize}
		...
		\item $W_{t} = $ investable wealth (endowment at time t \replacedTP{after consumption $c_t$ took place}{minus consumption $c_t$})
		\item  $U_{t}(W_{t+1}) = -e^{-aW_{t+1}-bESG_t}$
		\item $a > 0 $ (the \replacedTP{agent}{investor} has positive absolute risk aversion)
