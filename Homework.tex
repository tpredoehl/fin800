...
\begin{document}
	...
	\replacedTP{Additionally, as $r_{t+1}$ is normally distributed, we can observe that the argument of the exponential function is a linear combination of a random variable normally distributed. Hence, considering that the expectation of the exponential of a normal corresponds to the exponential of the random variable's mean plus half of the variance, it follows that the optimization problem can be rewritten as: \vspace{-3em}\\}{The exponential is normally distributed given $r_{t+1}$ is normally distributed. Since the expectation of a normal exponential equals the exponential of its mean plus one half of its variance:}
