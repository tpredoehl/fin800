...
\begin{document}
	...
	\begin{itemize}
			...
			\item \highlightTP{if $\mu_{x, ESG} < MAER$ exclude from universe}
			\item \highlightTP{if $\sigma_{ESG}^2 > \sigma_{MAER}^2$ exclude from universe}
			\item if $\sigma_{1,ESG} > \sigma_{2,ESG}$ and $\mu_{1, ESG} < \mu_{2, ESG}$ then $uni_1 < uni_2$
			\item if $(CE_{ESG_1} < CED_{ESG_2})$ then $uni_1 < uni_2$
			\item if $uni_1 < uni_2$ then $E[W_{1,t+1}] < E[W_{2,t+1}]$
		\end{itemize}
