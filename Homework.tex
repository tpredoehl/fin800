...
\begin{document}
	...
	Following the original setup of 1.2, \replacedTP{we can see now that the argument of the exponential is still a normal random variable. Indeed, such variable is represented as a linear combination of two normals: $W_{t+1}$\footnote{$W_{t+1}$ itself is a linear combination of the normal: $r_{t+1}$.} and $ESG_t$. Henceforth we have: }{the revised maximization program is an affine transformation of two normal random variables, $W_{t+1}$\footnote{$W_{t+1}$ itself is an affine transformation of a normal random variable: $r_{t+1}$.} and $ESG_t$. As before, the expectation of a normal exponential equals the mean plus one half of the variance of the exponential:}
	\begin{align*}
		\max_{(\theta_t)} E_t[U_{t}(W_{t+1})] &= E_t[-e^{(-aW_t) [1 + r_f + \theta_t(r_{t+1}-r_f)] - bESG_t}] \\
	...
	As a consequence of the above result, we can infer that the mean, the variance and the distribution of the optimal wealth will be unchanged.\todo{separated out}
